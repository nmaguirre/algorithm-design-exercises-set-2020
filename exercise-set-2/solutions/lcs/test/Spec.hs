...
spec = do
    describe "Longest Common Subsequence" $ do
        ...
        it "Test lcsDecrease with one empty list" $ do
                lcsDecrease [] "ACDBAC" `shouldBe` ([] :: String)
        it "Test lcsDecrease inputs with no element in common " $ do
                lcsDecrease "BCDAACD" "FGHHFG" `shouldBe` ([] :: String)
        it "Test lcsBf with equal list" $ do
                lcsDecrease "BCDAACD" "BCDAACD" `shouldBe` ("BCDAACD" :: String)                
